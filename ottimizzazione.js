...
class Scene3D {
  ...
  async loadModel() {
    try {
      ...
      const gltf = await loader.loadAsync('./management2.glb');
      ...
      console.error('Errore caricamento management.glb:', e);
    }
  }
  ...
  setupScrollAnimation() {
    // keyframes desktop
    const posKFDesktop = [
      { x: 10, y: -5,   z: 80 },{ x: 30, y: -2,   z: 60 },{ x: 30, y: -8,   z: 60 },
      { x: 35, y: -2.2, z: 40 },{ x: 18, y: -8,   z: 75 },{ x: 2.2, y: -1,   z: 97.2 },
      { x: 30, y: 0,    z: 80 },{ x: 60, y: 0,    z:100 },{ x: 19, y: 0,    z: 65 },{ x: 19, y: 0,    z: 65 }
    ];
    // keyframes mobile: shift X a sinistra di 10 unità
    const posKFMobile = [
      { x: -3, y: -9,   z: 70 },{ x: 6, y: -2,   z: 60 },{ x: -6, y: -8,   z: 40 },
      { x: -8, y: -2.2, z: 50 },{ x: 18, y: -8,   z: 75 },{ x: 2.2, y: -1,   z: 97.2 },
      { x: 30, y: 0,    z: 80 },{ x: 5, y: 0,    z:65 },{ x: 5, y: 0,    z: 45 },{ x: 0, y: 0,    z: 65 }
    ];

    const rotYKF = [-Math.PI*0.75, -Math.PI*0.5, -Math.PI*0.5, -Math.PI*0.45, -Math.PI*0.5,
                    -Math.PI*0.5, Math.PI*0.5, Math.PI*0.71, -Math.PI*0.5, -Math.PI*0.5];
    const rotXKF = [2, Math.PI*1.5, Math.PI*2.1, Math.PI*2.2, 0, 0, 0, Math.PI*0.1, 5, 0];
